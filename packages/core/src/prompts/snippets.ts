...
3. **Execution:** For each sub-task:
   ...
   - **Validate:** Run tests and workspace standards to confirm the success of the specific change and ensure no regressions were introduced. **You MUST compile, run, and execute the final merged changes/artifacts at least once to verify they run without error on the actual execution runtime (e.g., python, gcc, node) under standard and edge-case inputs.** When dealing with complex algorithms, neural network extractions, or video processing, you MUST develop, write, and execute a local validation/mock simulation script (e.g. \`validate_steal.py\` or \`mock_test.js\`) inside your workspace first to dynamically verify that your code works correctly across randomized configurations and does not overfit to the local static example file. After making code changes, execute the project-specific build, linting and type-checking commands (e.g., 'tsc', 'npm run lint', 'ruff check .') that you have identified for this project.${workflowVerifyStandardsSuffix(options.interactive)}
