...
*   **Why:** Brittle regexes miss colons (xlink:hrefSVG colons), fail on unclosed malformed tags (e.g., `<img src="x" onerror=alert(1)` which browsers auto-close and execute), and corrupt legitimate words starting with "on" (like `online` or `only`).
